...
updated_at: '2024-11-20 18:30:11'
labels:
  - 小程序
  - CSS
  - 前端
...
由于需要把原图隐藏，那免不了要设置 `overflow: hidden`。

然而，drop-shadow 有一个特性：

> 在 Chrome 浏览器下，如果一个元素的主体部分，无论以何种方式，只要在页面中不可见，其 drop-shadow 是不可见的；实体部分哪怕有 1px 可见，则 drop-shadow 完全可见。

源自[张鑫旭大佬博客](https://www.zhangxinxu.com/wordpress/2016/06/png-icon-change-color-by-css/)，据测试 Safari 也有相同表现。里面提到一个解决方案是用透明边框。
...
.icon {
  ...
  filter: drop-shadow(200rpx 0 0 #1f883d);
  transform: translateX(-200rpx);
  border-right: 100000px solid transparent; /* 给一个足够大的值 */
}
```
