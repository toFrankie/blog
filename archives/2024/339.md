...
updated_at: '2024-07-01 09:40:49'
labels:
  - 前端
  - JS
  - '2024'
...
![配图源自 Freepik](https://cdn.jsdelivr.net/gh/toFrankie/blog@main/images/2024/6/1719246988626.jpg)
...
> 得出一个结论：`setTimeout(fn, delay)` 的 `delay` 是最小开始执行时间，而且只会多不会少。
...
> [CodePen](https://codepen.io/tofrankie/pen/RwmEqwR)
...
据查，Chrome 有做“自动修正”的处理（[源码](https://source.chromium.org/chromium/chromium/src/+/main:third_party/WebKit/Source/platform/Timer.cpp;drc=e6d900fb6ed08dbd3a048899f38962ee75f4d8d0;l=162)），因此当它执行了 300 多次，甚至更多时，其漂移也很低，几乎可以忽略。尽管这种修正并不是规范所要求，但它应该是开发者想要的结果。
...
除此之外，当页面挂起后台时，为了省电和减少 CPU 占用，不同浏览器会采用一些[策略](https://developer.chrome.com/blog/timer-throttling-in-chrome-88)，暂停或延长定时器的 Delay Time。
...
在 `updateUI()` 之前处理，也便于准确计算出下一秒的时间轴时间。
...
但微信提供了 [`wx.getPerformance()`](https://developers.weixin.qq.com/miniprogram/dev/api/base/performance/wx.getPerformance.html) 方法，通过 `wx.getPerformance().now()` 可以得到一个类似 `window.performance.now()` 的结果，但不完全相同：

- `wx.getPerformance().now()` 返回自 1970 年 1 月 1 日开始以来的毫秒数。
- `window.performance.now()` 返回自 `performance.timeOrigin` 开始以来的毫秒数。

需要注意的是，**我目前不确定 `wx.getPerformance().now()` 日后是否会有变动，因为[小程序文档](https://developers.weixin.qq.com/miniprogram/dev/api/base/performance/wx.getPerformance.html)并未指出其返回值包含 `now` 方法，但实测开发工具和真机是可用的**。有点 [`wx.onAppRoute()`](https://developers.weixin.qq.com/community/develop/doc/000800ea3d83984dafc6d476551000) 的味道，虽然文档一直未提到过，但一直可用。


![](https://cdn.jsdelivr.net/gh/toFrankie/blog@main/images/2024/6/1719296814897.png)
