...
updated_at: '2024-07-28 12:28:49'
labels:
  - 前端
  - JS
  - '2024'
...
Page({
  ...
  renderCounter(timeLeft) {
    ...
    const html = `${dayLeft} days, ${hourLeft} hours, ${minuteLeft} minutes, ${secondLeft} seconds`
    this.setData({ countdownStr: html })
  }
})
```

> 测试下来，`wx.getPerformance().now()` 会受系统时钟影响，或许其内部就是用的 `Date.now()` 吧。

> 不用 setInterval 的另一个好处是不用 onUnload 时 clear 掉。
