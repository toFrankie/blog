...
updated_at: '2024-07-28 15:45:19'
labels:
  - 前端
  - JS
  - '2024'
...
小程序有个 [`wx.getPerformance().now()`](https://developers.weixin.qq.com/miniprogram/dev/api/base/performance/wx.getPerformance.html) 方法（文档未提到），它返回的是自 1970 年 1 月 1 日 0 点开始以来的毫秒数，调试发现其内部返回的就是 `Date.now()` 🙄
...
Page({
  ...
  onShow() {
    // TIPS: 由于 Date.now() 受系统时钟影响，需根据当前服务器时间、倒计时截止时间计算得出剩余秒数
    this.countdown(90)
  },
  ...
  renderCounter(timeLeft) {
    ...
    const html = `${dayLeft} days, ${hourLeft} hours, ${minuteLeft} minutes, ${secondLeft} seconds`
    this.setData({ formattedCountdown: html })
  }
})
```

> 不用 setInterval 的另一个好处是不用 onUnload 时手动 clear。
