...
updated_at: '2024-08-18 14:58:23'
labels:
  - 前端
  - JS
  - '2024'
...
> Each event loop has a [microtask queue](https://html.spec.whatwg.org/multipage/webappapis.html#microtask-queue), which is a queue of microtasks, initially empty.
...
- 一个 Event Loop 有一个 microtask queue。
- task queue 是一组 [task](https://html.spec.whatwg.org/multipage/webappapis.html#concept-task) 的[集合](https://infra.spec.whatwg.org/#ordered-set)，并不是[队列](https://infra.spec.whatwg.org/#queue)。
- microtask queue 不是 task queue。
- microtask 只是一种通俗的说法，它是通过特定算法创建的 task。

为便于区分，本文暂且将以下规范术语口语化，尽管这种说法不一定完全准确。
