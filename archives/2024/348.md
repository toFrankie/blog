...
updated_at: '2024-08-23 21:15:37'
labels:
  - 前端
  - JS
  - '2024'
...
假设 JavaScript 不是单线程的，设想一下：同时有两个线程对一个 button 操作，一个是添加事件，一个是删除元素。两种可能：

- 先删除元素，接着添加事件时抛出异常。
- 先添加事件，接着删除元素（等于白加了）。

最终结果是？哪个结果才是对的？都不对，正确与否视乎实际应用场景。

单线程可以不用考虑这种情况，操作变得有序、可控，背后的就是 Event Loop 机制。
