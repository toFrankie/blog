...
updated_at: '2024-08-24 16:50:11'
labels:
  - 前端
  - JS
  - '2024'
...
前面提到，task 是有 source（源），比如来自鼠标点击等。[排队](https://html.spec.whatwg.org/multipage/webappapis.html#queue-a-task)时，同 source 的 task 会被放入与该 source 相关的 task queue 里。假设鼠标事件的任务要优于其他任务，Event Loop 就可以优先在对应 source 的 task queue 中取出任务优先执行。规范里 Event Loop 执行[步骤](https://html.spec.whatwg.org/multipage/webappapis.html#event-loop-processing-model)并没有明确定义“出队”的规则，具体取决于浏览器的实现。
...
> 😲 天塌了，在此之前，我的认知是：一个 Event Loop 里有且只有一个任务队列，且它是一个常规意义的队列。虽说如此，如果只想了解 Event Loop 主要执行顺序，不深入浏览器究竟维护了多少个任务队列、浏览器如何决定下一任务，按原来的理解也问题不大。
...
> 题外话，习惯了更高刷新率的屏幕，每 16.7ms 刷新一次，也不见得很流畅，所以流畅也是相对的。
...
> 实际情况，Edge 和 Safari 浏览器将 requestAnimationFrame 回调放到 Paint 后面执行，这是非标准做法。也就是说，如果回调中涉及样式，用户要在下一帧才能看到变化。

> 是否已修复，暂未亲测，待验证。

现在 Event Loop，除了有 task queue（集合）、microtask queue（队列），还有一个 animation frame callbacks，它是一个映射（[ordered map](https://infra.spec.whatwg.org/#ordered-map)）。
