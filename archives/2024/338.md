...
updated_at: '2024-05-25 16:17:02'
labels:
  - Node.js
  - 前端
  - '2024'
...
> 由于 `.env` 文件可能会包含像密钥这类敏感信息，它不在版本控制范围内。如果是多人协作的项目，可以考虑添加类似 `.env.example` 模板到仓库里，以便其他成员清楚了解用到哪些环境变量。
...
> 说那么多，是为了不要混淆 `--env`、`--mode` 与 `process.env.NODE_ENV`。
