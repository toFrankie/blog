...
updated_at: '2024-05-19 19:34:14'
labels:
  - Node.js
  - 前端
  - '2024'
...
> 在命令前加上变量声明，它会传递给子进程。类似 `export` 的效果，但不完全相同，这种方式不会影响当前进程的同名变量（如果有的话）。[Simple Command Expansion](https://www.gnu.org/software/bash/manual/bash.html#Simple-Command-Expansion)
...
当然，以上环境变量仅可在编译时有效。要在业务代码中使用，还得借助类似 DefinePlugin 等插件处理，它们将会在编译时被替换为相应的字符串。
