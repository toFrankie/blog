...
updated_at: '2025-07-13 17:21:01'
labels:
  - '2025'
  - Tools
---
...
基本上都在 `package.json` 声明。

列举一些基本字段，完整字段说明看 [Extension properties](https://developers.raycast.com/information/manifest#extension-properties) 或 [Extension Schemas](https://www.raycast.com/schemas/extension.json)。
...
```json
{
  ...
  "contributors": ["someone"], // 对扩展作出过贡献的 Raycast 用户名列表
  "license": "MIT", // 协议
  "categories": ["Developer Tools"], //  // 扩展所属分类，至少一个
  "commands": [ // 命令列表
    {
      "name": "open-project", // 命名名称，在列表中需保持唯一。
      "title": "Open Project",
      "description": "Open Mini Program projects via WeChat DevTool.",
      "mode": "view"
    },
    {
      "name": "configure",
...
- **name**：不能与 Store 已有扩展重复。将用于发布后的扩展链接，比如： `https://www.raycast.com/tofrankie/wechat-devtool`
- **icon**：图标放在 `/assets` 目录。其他需要跟扩展一同打包的静态资源都要放在此目录下。
- **author**：你的 Raycast 用户名。注意不是 GitHub 用户名。
