...
updated_at: '2025-08-25 18:30:29'
labels:
  - '2025'
  - Tools
---
...
存放在 `metadata` 目录下，可通过 1 ~ 6 张截图。

Raycast 内置提供了截屏功能，在 Raycast Settings - Advanced - Window Capture 中设置快捷键（如 ⌥ + ⇧ + ⌘ + M）。

在 npm run dev 模式下启动扩展，按下快捷键，便可调出截屏窗口，并勾选上「Save to Metadata」，就会自动存入 `metadata` 目录，更多请看[这里](https://developers.raycast.com/basics/prepare-an-extension-for-store#screenshots)。
