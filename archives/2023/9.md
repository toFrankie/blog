...
updated_at: '2023-12-02 20:09:15'
labels:
  - Mac
  - Homebrew
  - '2017'
...
> 截止行文时间 Homebrew 最新版本为 `4.1.22`。

自 `4.0.0` 起，默认使用从 [Formulae.brew.sh](https://formulae.brew.sh/) 下载的 JSON 文件进行包的安装，而不是本地 `homebrew/core` 和 `homebrew/cask taps`。自动运行 brew update 的频率由 5min 改为 24h。

- 可以考虑使用 `brew untap homebrew/core` 和 `brew untap homebrew/cask` 来节省一些空间，除非你正在开发包。
- 可以重新考虑是否需要设置 `HOMEBREW_NO_AUTO_UPDATE`、`HOMEBREW_AUTO_UPDATE_SECS`、`HOMEBREW_NO_INSTALL_FROM_API` 了（如果设置过）。
- 可以考虑将 `HOMEBREW_API_DOMAIN` 设为指定镜像源（如果使用新方式）。
...
如果想沿用以前的安装方式，可以设置环境变量 `HOMEBREW_NO_INSTALL_FROM_API=1`。比如你是包开发者。

```shell
export HOMEBREW_NO_INSTALL_FROM_API=1
```

> Related Link: [Homebrew 4.0.0](https://brew.sh/2023/02/16/homebrew-4.0.0/).

### Homebrew 4.0 之前


> 如果你使用 3.3.0 ~ 3.6.0 之间的版本，想要使用全新的 JSON API 来安装包，可以设置环境变量 `HOMEBREW_INSTALL_FROM_API=1`。

```shell
export HOMEBREW_NO_AUTO_UPDATE=true   # 关闭 brew 每次执行命令时的自动更新
